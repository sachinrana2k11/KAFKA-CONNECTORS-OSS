...
class main:
    ...
    def attach_to_mqtt(self):
        while 1:
            if self.q.empty():
                print("No Data Avaialble- waiting for new data...")
                time.sleep(m.config["MQTT-CONFIG"]["MQTT_WAIT_TIME"])
            if not self.q.empty():
                in_data = self.q.get()
                self.mqtt.MQTT_Send_Data(in_data)
                print("Attaching to Mqtt Data Packet-:" + str(in_data))
...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        for x in range(1, m.my_variable + 1):
            ...
            obj = threading.Thread(target=m.attach_to_mqtt)
            obj.start()